...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  // INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi, I, R(rd) = src1 + imm);
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw, I, R(rd) = SEXT(src1 + SEXT(imm,32),32));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw, I, R(rd) = SEXT(src1<<(BITS(imm, 5, 0)), 32));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli, I, R(rd) = src1 >> (BITS(imm, 5, 0)));
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai, I, R(rd) = src1 >> (BITS(imm, 5, 0)));
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw, I, R(rd) = SEXT(SEXT(BITS(src1,31,0),32) >> (BITS(imm, 5, 0)),32));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4, s->dnpc = s->pc + imm;s->n_pc = s->pc;s->jar_pc = s->dnpc); // printf("%ld",SEXT((BITS(0xfe9ff0ef, 31, 31) << 20) | (BITS(0xfe9ff0ef, 30, 21) << 1) | (BITS(0xfe9ff0ef, 20, 20)<< 11) |(BITS(0xfe9ff0ef, 19, 12) << 12),21))
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw, R, R(rd) = SEXT(SEXT(BITS(src1, 31, 0),32) >> src2, 32));
